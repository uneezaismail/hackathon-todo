"""
Task model for the Todo application Phase V.
Defines the database schema for tasks with user isolation, recurring task support,
and event-driven architecture integration.

Phase V Additions:
- recurring_pattern: RRULE string for flexible recurrence patterns
- next_occurrence: Calculated next occurrence timestamp (UTC)
- Event publishing on task mutations
"""
...
class TaskBase(SQLModel):
    ...
    Inherits from SQLModel to enable proper Pydantic validation.
    """
    title: str = Field(
        min_length=1,
        max_length=200,
...
class Task(TimestampMixin, table=True):
    ...
    # TaskBase fields (title, description, completed, priority, due_date)
    title: str = Field(
        min_length=1,
        max_length=200,
        description="Task title (1-200 characters)"
    ...
    # Phase V: RRULE-based recurring task fields
    recurring_pattern: Optional[str] = Field(
        default=None,
        max_length=500,
        description="RRULE pattern string (e.g., DAILY, WEEKLY, FREQ=DAILY;INTERVAL=1)"
    )
    ...
    # Timestamp fields (must be defined explicitly, not inherited)
