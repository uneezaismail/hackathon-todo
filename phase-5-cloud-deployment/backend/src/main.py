...
- Health check endpoints (liveness, readiness, Dapr, Kafka)
- Prometheus metrics endpoint
- Global exception handlers for standardized error responses
- Event-driven architecture support via Dapr
"""
...
from .api.v1 import tasks, chatkit, health, metrics
...
app = FastAPI(
    title="Todo API - Phase V (Cloud Deployment)",
    description="FastAPI backend for Todo application with AI chatbot, JWT authentication, and event-driven architecture via Dapr",
    version="0.2.0",
    docs_url="/docs",
    redoc_url="/redoc",
)
...
# Phase V: Health check endpoints (comprehensive)
...
async def readiness_check():
    ...
    Returns:
        dict: Readiness status and timestamp

    Raises:
        HTTPException 503: If database is unreachable or configuration is invalid
    """
...
async def root():
    ...
    """
    return {
        "message": "Todo API - Phase 4 Backend",
        "status": "ready",
        "docs": "/docs",
        "health": "/api/health",
    }
