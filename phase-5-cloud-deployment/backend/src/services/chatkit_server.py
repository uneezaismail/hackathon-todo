...
class TaskChatKitServer(ChatKitServer):
    ...
    async def respond(
        ...
        """
        import asyncio  # Import at function level for use throughout
        ...
        async with self.mcp_server:
            # CRITICAL FIX: Wait for MCP server to be fully initialized
            # The context manager starts the subprocess but doesn't wait for it to be ready
            # We need to ensure the server is connected before running the agent
            import asyncio
            await asyncio.sleep(0.5)  # Give server 500ms to initialize
