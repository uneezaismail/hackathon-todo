...
def list_tasks(
    ...
    try:
        ...
        for task in tasks:
            ...
            if task.is_recurring and hasattr(task, 'recurrence_type'):
                task_dict["recurrence_type"] = task.recurrence_type.value if hasattr(task.recurrence_type, 'value') else str(task.recurrence_type)
                task_dict["recurrence_interval"] = task.recurrence_interval
...
def complete_task(
    ...
    try:
        ...
        # Use RecurringService to complete task and generate next occurrence if recurring
        ...
        # Add next occurrence info if generated
        if next_occurrence:
            response["next_occurrence"] = {
                "id": str(next_occurrence.id),
...
# =============================================================================
# Phase 4: Recurring Tasks MCP Tools
# =============================================================================


def parse_recurrence_pattern(text: str) -> dict:
    """
    Parse natural language recurrence patterns into structured configuration.

    Args:
        text: User input text containing recurrence patterns

    Returns:
        dict: Recurrence configuration or empty dict if no pattern found
            - is_recurring (bool): True if recurring pattern found
            - recurrence_type (str): "daily", "weekly", "monthly", "yearly"
            - recurrence_interval (int): Interval between occurrences
            - recurrence_days (str): Days for weekly recurrence (e.g., "mon,wed,fri")

    Examples:
        >>> parse_recurrence_pattern("add a task every day")
        {"is_recurring": True, "recurrence_type": "daily", "recurrence_interval": 1}
        >>> parse_recurrence_pattern("remind me every monday")
        {"is_recurring": True, "recurrence_type": "weekly", "recurrence_interval": 1, "recurrence_days": "mon"}
        >>> parse_recurrence_pattern("every 2 weeks")
        {"is_recurring": True, "recurrence_type": "weekly", "recurrence_interval": 2}
    """
    text_lower = text.lower()

    # Daily patterns: "every day", "daily", "everyday"
    if re.search(r'\bevery\s*day\b|\bdaily\b|\beveryday\b', text_lower):
        return {
            "is_recurring": True,
    ...
    # Every N days pattern: "every 3 days"
    days_match = re.search(r'\bevery\s+(\d+)\s+days?\b', text_lower)
    if days_match:
        return {
            "is_recurring": True,
            "recurrence_type": "daily",
            "recurrence_interval": int(days_match.group(1)),
        }

    # Weekly patterns: "every week", "weekly"
    if re.search(r'\bevery\s*week\b|\bweekly\b', text_lower):
        return {
            "is_recurring": True,
    ...
    # Every N weeks pattern: "every 2 weeks", "every other week"
    weeks_match = re.search(r'\bevery\s+(\d+)\s+weeks?\b', text_lower)
    if weeks_match:
        return {
            "is_recurring": True,
            "recurrence_type": "weekly",
            "recurrence_interval": int(weeks_match.group(1)),
        }
    if re.search(r'\bevery\s+other\s+week\b', text_lower):
        return {
            "is_recurring": True,
            "recurrence_type": "weekly",
    ...
    # Specific weekday patterns: "every monday", "every mon and wed"
    weekday_map = {
        "monday": "mon", "mon": "mon",
        "tuesday": "tue", "tue": "tue",
    ...
    weekday_pattern = r'\bevery\s+(monday|tuesday|wednesday|thursday|friday|saturday|sunday|mon|tue|wed|thu|fri|sat|sun)'
    weekday_match = re.search(weekday_pattern, text_lower)
    if weekday_match:
        day = weekday_match.group(1)
        days = [weekday_map.get(day, day[:3])]

        # Check for additional days with "and"
        additional_days = re.findall(
            r'\b(monday|tuesday|wednesday|thursday|friday|saturday|sunday|mon|tue|wed|thu|fri|sat|sun)\b',
            text_lower
        )
        for d in additional_days:
            short_day = weekday_map.get(d, d[:3])
    ...
    # Monthly patterns: "every month", "monthly"
    if re.search(r'\bevery\s*month\b|\bmonthly\b', text_lower):
        return {
            "is_recurring": True,
    ...
    # Every N months pattern: "every 3 months"
    months_match = re.search(r'\bevery\s+(\d+)\s+months?\b', text_lower)
    if months_match:
        return {
            "is_recurring": True,
            "recurrence_type": "monthly",
            "recurrence_interval": int(months_match.group(1)),
        }

    # Yearly patterns: "every year", "yearly", "annually"
    if re.search(r'\bevery\s*year\b|\byearly\b|\bannually\b', text_lower):
        return {
            "is_recurring": True,
...
def add_recurring_task(
    user_id: str,
    title: str,
    recurrence_type: Literal["daily", "weekly", "monthly", "yearly"],
    description: Optional[str] = None,
    priority: Optional[str] = None,
    due_date: Optional[str] = None,
    ...
    """
    Create a recurring task that repeats on a schedule.

    MCP Tool Contract:
    - Purpose: Create a task that automatically generates new occurrences
    - Stateless: All state persisted to database
    - User Isolation: Enforced via user_id parameter
    ...
    Args:
        user_id: User's unique identifier (string UUID from Better Auth)
        title: Task title (required, max 200 characters)
        recurrence_type: Recurrence pattern type (required)
            - "daily": Repeats every N days
            - "weekly": Repeats every N weeks (optionally on specific days)
            - "monthly": Repeats every N months on the same day
            - "yearly": Repeats every N years on the same day
        description: Task description (optional, max 1000 characters)
        priority: Task priority level (optional: "low", "medium", "high")
        due_date: First due date (optional, ISO 8601 or natural language)
        recurrence_interval: Number of periods between occurrences (default: 1)
            - e.g., interval=2 with type="weekly" = every 2 weeks
        recurrence_days: Days for weekly recurrence (optional)
            - Comma-separated: "mon,wed,fri"
            - Only applies when recurrence_type="weekly"
        recurrence_end_date: Optional end date for recurrence (ISO 8601)
        max_occurrences: Optional maximum number of occurrences

    Returns:
        dict: Recurring task creation result
            - task_id (str): Created task UUID
            - status (str): "created"
            - title (str): Task title
            - is_recurring (bool): True
            - recurrence_type (str): Recurrence pattern type
            - recurrence_interval (int): Interval between occurrences

    Example:
        >>> add_recurring_task(user_id="user-123", title="Weekly meeting", recurrence_type="weekly", recurrence_days="mon")
        {"task_id": "...", "status": "created", "is_recurring": True, "recurrence_type": "weekly"}
    """
    import sys
    from datetime import date
    from src.models.task import TaskCreate, PriorityType, RecurrenceType

    print(f"[MCP TOOL] add_recurring_task CALLED: user_id={user_id}, title={title}, type={recurrence_type}", file=sys.stderr, flush=True)
    ...
    if recurrence_end_date:
        try:
            parsed_end_date = date.fromisoformat(recurrence_end_date)
        except ValueError:
            print(f"[MCP TOOL] Invalid recurrence_end_date format: {recurrence_end_date}", file=sys.stderr, flush=True)
    ...
    try:
        ...
        # Map recurrence type string to RecurrenceType enum
        recurrence_type_map = {
        ...
        recurrence_enum = recurrence_type_map.get(recurrence_type)

        if not recurrence_enum:
            return {
                "task_id": None,
                "status": "error",
                "message": f"Invalid recurrence_type: {recurrence_type}. Must be daily, weekly, monthly, or yearly.",
            }
...
def list_recurring_tasks(
    ...
    MCP Tool Contract:
    - Purpose: Show all recurring task configurations
    - Stateless: Queries database on each invocation
    - User Isolation: Enforced via user_id parameter

    Args:
        user_id: User's unique identifier (string UUID from Better Auth)
        status: Filter recurring tasks (default: "active")
            - "active": Only recurring tasks that are not completed
            - "completed": Only completed recurring tasks
            - "all": All recurring tasks

    Returns:
        dict: List of recurring task patterns
            - tasks (list): Array of recurring task objects
            - count (int): Number of recurring tasks

    Example:
        >>> list_recurring_tasks(user_id="user-123")
        {"tasks": [{"id": "...", "title": "Weekly meeting", "recurrence_type": "weekly", ...}], "count": 1}
    """
    from src.services.task_service import TaskService
    ...
    try:
        ...
        # Get tasks with is_recurring=True (use limit as filter workaround)
        tasks, total = TaskService.get_user_tasks(
            session=session,
            user_id=user_id,
            completed=completed_filter,
        ...
        # Filter to only recurring tasks (parent tasks, not instances)
        recurring_tasks = [
            {
                "id": str(task.id),
                "title": task.title,
                ...
                "recurrence_type": task.recurrence_type.value if hasattr(task.recurrence_type, 'value') else task.recurrence_type,
        ...
        return {
            "tasks": [],
            "count": 0,
            "message": f"Failed to list recurring tasks: {str(e)}",
        }
...
def stop_recurring_task(
    ...
    MCP Tool Contract:
    - Purpose: Disable future occurrence generation for a recurring task
    - Stateless: Updates database and returns result
    - User Isolation: Enforced via user_id parameter
    ...
    Returns:
        dict: Stop recurrence result
            - task_id (str): Updated task ID
            - status (str): "stopped"
            - title (str): Task title

    Example:
        >>> stop_recurring_task(user_id="user-123", task_id="550e8400-...")
        {"task_id": "...", "status": "stopped", "title": "Weekly meeting"}
    """
    from uuid import UUID
    from src.services.task_service import TaskService
    from src.services.recurring_service import RecurringService
    ...
    try:
        ...
        updated_task = RecurringService.stop_recurrence(session, task, user_id)
        ...
        return {
            "task_id": task_id,
            "status": "error",
            "message": f"Failed to stop recurrence: {str(e)}",
        }
